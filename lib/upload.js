...
class BaseUpload {
  constructor (file, options) {
    ...
    // An incremental checksum generator, this is instantiated if options.enableChecksum is provided
    this._shaObj = null

    // The final SHA-256 checksum
    this.sha256sum = null

    // onProgress time interval helper
    this._progressDate = new Date()

    // onProgress time interval (default 1 sec)
    this._progressInterval = 1000
  }
  ...
  _emitSuccess () {
    ...
    // if (this._shaObj) {
    //   this.sha256sum = this._shaObj.getHash('HEX')
    // }
  ...
  _emitProgress (bytesSent, bytesTotal) {
    if (typeof this.options.onProgress === 'function') {
      if (((new Date()) - this._progressDate) / this._progressInterval >= 1) {
        this._progressDate = new Date()
  ...
  _createUpload () {
    ...
    if (this.options.enableChecksum) {
      ...
      this._shaObj = new jsSHA('SHA-256', 'ARRAYBUFFER')
    }
  ...
  _addChunkToRequest (req) {
    ...
    return this._source.slice(start, end)
      .then(async ({ value, done }) => {
        ...
        if (this.options.enableChecksum && this._shaObj) {
          await value.arrayBuffer().then((chunkBuffer) => {
            if (this._retryAttempt === 0) {
              this._shaObj.update(chunkBuffer)
            }

            // is it last chunk?
            if (this._size - this._offset <= this.options.chunkSize) {
              this.sha256sum = this._shaObj.getHash('HEX')
              req.setHeader('sha256sum', this.sha256sum)
            }
          })
        }
        if (value === null) {
...
function shouldRetry (err, retryAttempt, options) {
  ...
  // - the error is server error (i.e. not a status 4xx except a 401, 409 or 423) or
  ...
  return (!inStatusCategory(status, 400) || status === 401 || status === 409 || status === 423) && isOnline()
}
